...
void CaptureLoop() {
    ...
    for (int attempt = 1; attempt <= MAX_RETRIES; ++attempt) {
        ...
        if (rawSocket == INVALID_SOCKET) {
            ...
            char logs[128];
            sprintf_s(logs,sizeof(logs),"socket creation failed (attempt %d/%d), error: %d", attempt, MAX_RETRIES, WSAGetLastError());
            LogRecord::WriteLog(L".\\Logs\\LogFiles\\NetworkGuard.log","[WARNING]","[NetworkGuard]",logs);
            ...
            LogRecord::WriteLog(L".\\Logs\\LogFiles\\NetworkGuard.log","[ERROR]","[NetworkGuard]","socket creation failed");
            return;
        }
        ...
        if (!bound) {
            ...
            char logs[64];
            sprintf_s(logs,sizeof(logs),"bind failed (attempt %d/%d)", attempt, MAX_RETRIES);
            LogRecord::WriteLog(L".\\Logs\\LogFiles\\NetworkGuard.log","[WARNING]","[NetworkGuard]",logs);
            ...
            LogRecord::WriteLog(L".\\Logs\\LogFiles\\NetworkGuard.log","[ERROR]","[NetworkGuard]","bind failed");
            return;
        }
        ...
        if (WSAIoctl(rawSocket, SIO_RCVALL, &dwRcvAll, sizeof(dwRcvAll),
            ...
            printf("[WARNING] WSAIoctl SIO_RCVALL failed (attempt %d/%d), error: %d\n",
                   attempt, MAX_RETRIES, WSAGetLastError());
            char logs[128];
            sprintf_s(logs,sizeof(logs),"WSAIoctl SIO_RCVALL failed (attempt %d/%d), error: %d", attempt, MAX_RETRIES, WSAGetLastError());
            LogRecord::WriteLog(L".\\Logs\\LogFiles\\NetworkGuard.log","[WARNING]","[NetworkGuard]",logs);
            closesocket(rawSocket);
            if (attempt < MAX_RETRIES) {
                std::this_thread::sleep_for(std::chrono::seconds(RETRY_DELAY_SEC));
            ...
            LogRecord::WriteLog(L".\\Logs\\LogFiles\\NetworkGuard.log","[ERROR]","[NetworkGuard]","WSAIoctl failed");
            return;
        }
...
void ProcessPacket(const unsigned char* packet, int len) {
    ...
    try {
        ...
        char logs[128];
        sprintf_s(logs,sizeof(logs),"ProcessPacket exception: %s, GetLastError=%lu", e.what(), GetLastError());
        LogRecord::WriteLog(L".\\Logs\\LogFiles\\NetworkGuard.log","[ERROR]","[NetworkGuard]",logs);
    } catch (...) {
        printf("[ERROR] ProcessPacket unknown exception, GetLastError=%lu\n", GetLastError());
        char logs[128];
        sprintf_s(logs,sizeof(logs),"ProcessPacket unknown exception, GetLastError=%lu", GetLastError());
        LogRecord::WriteLog(L".\\Logs\\LogFiles\\NetworkGuard.log","[ERROR]","[NetworkGuard]",logs);
    }
}
...
LONG WINAPI VectoredExceptionHandler(EXCEPTION_POINTERS* pExceptionInfo) {
    ...
    char logs[256];
    sprintf_s(logs,sizeof(logs),
        "[FATAL] Unhandled SEH exception detected.   ExceptionCode: 0x%08X.  ExceptionFlags: 0x%08X.  ExceptionAddress: %p.", 
        pExceptionInfo->ExceptionRecord->ExceptionCode, pExceptionInfo->ExceptionRecord->ExceptionFlags,pExceptionInfo->ExceptionRecord->ExceptionAddress);
    LogRecord::WriteLog(L".\\Logs\\LogFiles\\NetworkGuard.log","[ERROR]","[NetworkGuard]",logs);
...
int main() {
    ...
    if (!vehHandle) {
        ...
        LogRecord::WriteLog(L".\\Logs\\LogFiles\\NetworkGuard.log","[WARNING]","[NetworkGuard]","AddVectoredExceptionHandler failed");
    }

    if(!EnableDebugPrivilege()){
        LogRecord::WriteLog(L".\\Logs\\LogFiles\\NetworkGuard.log","[ERROR]","[NetworkGuard]","Failed to enable debug privilege.");
    }
    if(!SetProcessCritical(true)){
        LogRecord::WriteLog(L".\\Logs\\LogFiles\\NetworkGuard.log","[ERROR]","[NetworkGuard]","Failed to set process critical.");
    }
    ...
    try {
        ...
        if (!g_hExitEvent) {
            ...
            LogRecord::WriteLog(L".\\Logs\\LogFiles\\NetworkGuard.log","[ERROR]","[NetworkGuard]","Failed to create exit event.");
            SetProcessCritical(false);
            return 1;
        }
        ...
        if (!RetryOperation([]() -> bool {
                ...
                LogRecord::WriteLog(L".\\Logs\\LogFiles\\NetworkGuard.log","[ERROR]","[NetworkGuard]","WSAStartup failed.");
                SetProcessCritical(false);
                return 1;  // 最终失败则退出
        }
        if (!RetryOperation([]() -> bool {
            ...
            WSACleanup();
            LogRecord::WriteLog(L".\\Logs\\LogFiles\\NetworkGuard.log","[ERROR]","[NetworkGuard]","GetLocalIPs failed.");
            SetProcessCritical(false);
            return 1;
        }
        ...
        char logs[64];
        sprintf_s(logs,sizeof(logs),"Main exception: %s, GetLastError=%lu", e.what(), GetLastError());
        LogRecord::WriteLog(L".\\Logs\\LogFiles\\NetworkGuard.log","[ERROR]","[NetworkGuard]",logs);
        CleanupWFP();
        if (g_pipeHandle != INVALID_HANDLE_VALUE) CloseHandle(g_pipeHandle);
        WSACleanup();
        ...
        char logs[64];
        sprintf_s(logs,sizeof(logs),"Main unknown exception, GetLastError=%lu", GetLastError());
        LogRecord::WriteLog(L".\\Logs\\LogFiles\\NetworkGuard.log","[ERROR]","[NetworkGuard]",logs);
        CleanupWFP();
        if (g_pipeHandle != INVALID_HANDLE_VALUE) CloseHandle(g_pipeHandle);
        WSACleanup();
