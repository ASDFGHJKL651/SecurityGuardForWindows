/*
CMDanalyzer_forWinPE.cpp
分析层

在WinPE环境中分析CMD/PowerShell脚本，评估并处理恶意代码

g++编译:
cd %g++Path%
g++ -fdiagnostics-color=always -g "%SourceCodePath%\CMDanalyzer_forWinPE.cpp" -o "%ExecutableForWinPEPath%\CMDanalyzer_forWinPE.exe"
...
public:
    ...
    void initRules() {
        ...
        // InstallUtil, regsvr32 /i:, msbuild, csc, rundll32 javascript:, wmic /node:, schtasks /xml
        ...
        // RootKit 攻击检测规则
        // 驱动/服务操纵
        rules.push_back({R"(sc\s+(?:start|create|config)\s+.*driver)", 7, "Service/driver manipulation (RootKit)", "T1543.003"});
        rules.push_back({R"(net\s+start\s+.*driver)", 6, "Start service/driver (RootKit)", "T1543.003"});
        rules.push_back({R"(devcon\s+(?:install|update|enable|disable))", 6, "Devcon device driver manipulation (RootKit)", "T1068"});
        rules.push_back({R"(rundll32\s+setupapi\.dll,\s*InstallHinfSection)", 7, "Install HINF section (driver installation)", "T1068"});
        // 文件隐藏
        rules.push_back({R"(attrib\s+\+h\s+\+s)", 4, "Hide file with system and hidden attributes (RootKit)", "T1564.001"});
        rules.push_back({R"(attrib\s+\+r\s+\+h\s+\+s)", 5, "Set read-only + hidden + system (RootKit)", "T1564.001"});
        // 系统行为修改
        rules.push_back({R"(fsutil\s+behavior\s+set)", 5, "FSUtil system behavior modification (RootKit)", "T1489"});
        rules.push_back({R"(bcdedit\s+/set\s+{current}\s+debug)", 6, "Enable kernel debugging (RootKit)", "T1007"});
        rules.push_back({R"(bcdedit\s+/set\s+{current}\s+testsigning)", 6, "Enable test signing (RootKit)", "T1007"});
        // 注册表服务路径篡改
        rules.push_back({R"(reg\s+add\s+.*\\Services\\.*\\ImagePath)", 7, "Modify service image path (RootKit persistence)", "T1543.003"});
        rules.push_back({R"(reg\s+add\s+.*\\Services\\.*\\Start\s+/\s*[dv])\s+0x[23])", 7, "Set service start type to auto or boot (RootKit)", "T1543.003"});
        // 进程注入/隐藏（常见工具名）
        rules.push_back({R"((?:GMER|IceSword|Darkspy|RootkitRevealer|SophosAntiRootkit)\.exe)", 5, "Rootkit detection/removal tool (potential misuse)", "T1068"});
        // 加载驱动程序通用模式
        rules.push_back({R"(sc\s+start\s+\S+\.sys)", 6, "Start .sys driver (RootKit)", "T1543.003"});
        rules.push_back({R"(net\s+start\s+\S+\.sys)", 6, "Start .sys driver via net (RootKit)", "T1543.003"});
        // 修改系统时间 / 关闭防御
        rules.push_back({R"(wmic\s+timezone\s+set)", 4, "Modify timezone (may evade logging)", "T1499"});
        rules.push_back({R"(net\s+stop\s+(?:winDefend|MsMpSvc|SecurityHealthService))", 7, "Stop security services (RootKit)", "T1562.001"});
        // 隐藏进程（通过 taskkill 结束关键进程）
        rules.push_back({R"(taskkill\s+/f\s+/im\s+(?:explorer|winlogon|csrss))", 7, "Kill critical system process (destructive)", "T1489"});
        // 写入内核内存（罕见）
        rules.push_back({R"(debug\s+.*\.sys)", 5, "Debug .sys driver (potential kernel manipulation)", "T1068"});
    }
...
public:
    ...
    Report analyze(const std::string& filePath) {
        ...
        std::unordered_set<std::string> tools = {
            ...
            "installutil", "msbuild", "csc", "devcon", "fsutil", "bcdedit"
        };
        ...
        report.hasValidSignature = false; // 忽略签名
        ...
        double rawConfidence = static_cast<double>(report.totalScore) / 120.0;
    ...
    void printReport(const Report& report) const {
        std::cout << "\n========== RootKit Batch File Detection Report (v3.0) ==========\n";
...
void printUsage(const char* progName) {
    std::cout << "Usage: " << progName << " <target.bat|target.cmd> [--withoutUi|--onlywithUi] [--XML]\n";
...
bool ForceDeleteFile(const std::string& filePath) {
    // 构建删除命令：del /f /q /a <文件>
    std::string cmdLine = "cmd /c del /f /q /a \"" + filePath + "\"";
    ...
    if (success) {
        ...
        DWORD err = GetLastError();
        std::cerr << "Failed to delete file (CreateProcess error: " << err << ")" << std::endl;
        return false;
    }
}
...
int main(int argc, char* argv[]) {
    std::cout << "RootKitBatDetector v3.0 - RootKit Batch File Detection & Auto-Deletion\n";
    if (argc < 2) {
        printUsage(argv[0]);
        return 1;
    }
    ...
    if (report.totalScore >= 40) {
        std::cout << "\n[!] Score >= 40, initiating forced deletion..." << std::endl;
        bool deleted = ForceDeleteFile(targetFile);
        if (deleted) {
            std::cout << "[+] File successfully deleted.\n";
        } else {
            std::cerr << "[-] File deletion failed. Please check permissions or if file is in use.\n";
        }
    } else {
        std::cout << "\n[+] Score below 40, no deletion performed.\n";
    }
