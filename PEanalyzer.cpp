...
g++编译:
cd %g++Path%
g++.exe -fdiagnostics-color=always -g "%SourceCodePath%\PEanalyzer.cpp" -o "%ExecutablePath%\PEanalyzer.exe" -lwintrust -lcrypt32 -lws2_32 -mwindows
    ...
    for (size_t i = 0; i + 6 < size; ++i) {
        if (data[i] == 0xB8) {
            if (i + 6 < size && data[i+5] == 0x0F && data[i+6] == 0x05) {
                if(addedScore < 24){addedScore += 12;}
...
static void analyze_entry_point_bytes(const BYTE* entryData, size_t len,
    ...
    for (size_t i = 0; i + 3 < len; ++i) {
        if (entryData[i] == 0x31 && (entryData[i+1] == 0x04 || entryData[i+1] == 0x1C) && entryData[i+2] == 0x0F) {
            ...
            if (hasInc && hasLoop) {
                if(addedScore < 20){addedScore += 10;}
...
bool AnalyzePE(const BYTE* base, size_t baseSize, const std::wstring& filePath, AnalysisResult& result, int depth = 0) {
    ...
    if (isSystemDir && isMicrosoftSigner) {
        ...
        result.info.push_back("Path context: " + loc + " directory with no valid signature, score increased from " + std::to_string(oldScore) + " to " + std::to_string(score) + " (*1.2)");
    ...
    if(hasValidSig){
        score = (int)(score * 0.5);
        result.info.push_back("Valid signature present, score reduced by 50% (*0.5)");
    }
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Risk level: ";
    if (result.score >= 90) std::cout << "HIGH";
    else if (result.score >= 60) std::cout << "MEDIUM";
    else if (result.score >= 40) std::cout << "SAFE";
    else std::cout << "LOW";
    std::cout << std::endl;

    if (result.score >= 90) std::cout << ">>> The file is LIKELY MALICIOUS." << std::endl;
    ...
    //  管道通信（原逻辑） 
    if (result.score >= 90 && withUi == 2) {
        MessagetoControlCenter_by_PEanalyzer message = {};
    ...
    if (writeXML) {
        ...
        // 将文件路径中的非法字符替换为 '_'
        std::wstring safeName = filePathW;
        ...
        std::wstring xmlPath = L".\\Logs\\" + safeName + L".xml";

        // 构建 XML 内容（宽字符串）
        std::wstring xmlContent;
        xmlContent += L"<analysis>\n";
        xmlContent += L"  <score>" + std::to_wstring(result.score) + L"</score>\n";
        xmlContent += L"  <details>\n";

        // 将 info 和 warnings 转换为宽字符串并添加节点
        auto addText = [&](const std::vector<std::string>& vec, const std::wstring& tag) {
            for (const auto& item : vec) {
                int wlen = MultiByteToWideChar(CP_ACP, 0, item.c_str(), -1, NULL, 0);
                std::wstring witem(wlen, L'\0');
                MultiByteToWideChar(CP_ACP, 0, item.c_str(), -1, &witem[0], wlen);
                witem.pop_back(); // 移除末尾 null
                xmlContent += L"    <" + tag + L"><![CDATA[" + witem + L"]]></" + tag + L">\n";
            }
        };
        addText(result.info, L"info");
        addText(result.warnings, L"warning");

        xmlContent += L"  </details>\n";
        xmlContent += L"</analysis>\n";

        // 转换为 UTF-8
        int utf8Len = WideCharToMultiByte(CP_UTF8, 0, xmlContent.c_str(), -1, NULL, 0, NULL, NULL);
        std::string utf8Xml(utf8Len, '\0');
        WideCharToMultiByte(CP_UTF8, 0, xmlContent.c_str(), -1, &utf8Xml[0], utf8Len, NULL, NULL);
        if (!utf8Xml.empty()) utf8Xml.pop_back(); // 去掉结尾的 '\0'

        // 写入文件
        HANDLE hXml = CreateFileW(xmlPath.c_str(), GENERIC_WRITE, 0, NULL,
                                  CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
        if (hXml != INVALID_HANDLE_VALUE) {
            DWORD written;
            WriteFile(hXml, utf8Xml.c_str(), (DWORD)utf8Xml.size(), &written, NULL);
            CloseHandle(hXml);
        }
    }
